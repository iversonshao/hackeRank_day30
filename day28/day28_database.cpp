...
bool checkFirstName(const string &name){
    ...
    return name.size() <= 20 && regex_match(name, pattern);
}
bool checkEmail(const string &email){
    static regex pattern("[a-z\\.]+@gmail\\.com");
    return email.size() <= 50 && regex_match(email, pattern);
}
int main(){
    string N_temp;
    getline(cin, N_temp);
    ...
    sort(users.begin(), users.end(), [](const user_info_t &a, const user_info_t &b){
        return a.firstName < b.firstName;
    });
    for_each(users.begin(), users.end(), [](const user_info_t &user){
